...
import dummyUser from "../pojos/userModel"
...
export const loginService = async (email: string, password: string) => {
    try {
      const users = await dummyUser.findOne({ email });
...
try{
  const users = await dummyUser.findOne({ email: iuser.email });
